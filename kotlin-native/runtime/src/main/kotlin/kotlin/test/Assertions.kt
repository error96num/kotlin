...
private fun <T : Throwable> checkResultIsFailureImpl(exceptionClass: KClass<T>, lazyMessage: () -> String?, blockResult: Result<Any?>): T {
    blockResult.fold(
            onSuccess = { v ->
                asserter.fail(messagePrefix(lazyMessage()) + "Expected an exception of ${exceptionClass.qualifiedName} to be thrown, ${formatResultMessage(v)}.")
            },
            onFailure = { e ->
                ...
                asserter.fail(messagePrefix(lazyMessage()) + "Expected an exception of ${exceptionClass.qualifiedName} to be thrown, but was $e", e)
            }
    )
}
